...
def fetch_neighbours(userid, direction="in"):
    ...
    if os.path.isfile(fname):
        print(str(userid) + " had already been fetched\n")
...
def api_neighbours(ego_screenname, direction = "in"):
    ...
    # Neighbours ids
    neighbours = api_neighbours_ids(ego, direction= direction)
    print("Ego neighbours:", len(neighbours))

    with open(os.path.join(PATHS[direction], str(ego)), 'w') as f:
        csv.writer(f).writerow(neighbours)
...
def api_neighbours_screen_names(ego_screenname, direction="in"):
    ...
    fname = os.path.join(PATHS[direction], str(ego))
...
def graph_ego(ego_screenname, direction="in"):
...
if __name__ == '__main__':
    ...
    if function == 'api_followers_graph':    
        graph_ego(screen_name, direction = "in")
